...
### 0. Never Start Without Permission
- **ALWAYS ask before building** - Don't start implementing without explicit "yes, build it"
- Present options, explain trade-offs, THEN wait for decision
- "Should I start building?" is mandatory before any major work
- Exception: Small fixes/clarifications that were explicitly requested

### 1. Deep Research & Best Solution First
...
- **ALWAYS look for the simplest, best solution first**
- Ask: "What data/tools already exist that solve this?"
- Consider the hybrid architecture (Shopify theme + Next.js)
- Don't default to complex patterns when a simple bridge exists
- Think from the user's specific setup, not textbook solutions
...
1. **Map what already exists** - What data, tools, APIs are already available in the current setup?
2. **Start from the simplest** - What's the fewest-moving-parts solution?
3. **Think hybrid** - RUCLO is Shopify theme + custom Next.js. Leverage BOTH sides.
4. **Check platform-native solutions** - Shopify Liquid, metafields, ScriptTags, App Proxy - use what Shopify gives you before building custom.
5. **Then escalate complexity** - Only go to OAuth, custom auth, external services if the simple path truly won't work.
6. **Compare at least 3 approaches** - Always. Table format. Trade-offs explicit.

### Common Mistake to Avoid:
- **Headless tunnel vision** - Don't forget Liquid templates have access to `customer`, `cart`, `shop` objects. The theme is a powerful tool, not just a visual layer.
- **Over-engineering** - MDP means the simplest robust solution, not the most architecturally pure one.
- **Defaulting to "standard" patterns** - This project's hybrid setup is unique. Solutions should fit THIS project, not a textbook.
